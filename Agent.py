...
def calc_speed(file_path: str,save_path: str = "speed_result.csv") -> str:
    ...
    for player_id in player_ids:
        for i in range(1, len(frames)):
            prev_pos = frames[i-1][player_id]
            curr_pos = frames[i][player_id]    
            x1, y1 = prev_pos
            x2, y2 = curr_pos
...
@tool
def calc_distance_with_player(player: str,target_player: str,positions: list, target_position:list,file_path: str) -> str:
    """
     读取json文件,分析所有帧球员与篮筐的距离,并保存为CSV文件。
    Args:
        file_path (str): 包含球员位置的JSON文件路径。
    ...
    distance_df.to_csv(f"{player}_{target_player}_distance.csv", index=False)
    return f"与球员的距离数据已保存到 {f"{player}_{target_player}_distance.csv"}"
@tool
...
agent = create_react_agent(
    ...
    tools=[read_csv,calc_speed,calc_distance_with_basket,calc_distance_with_player,detect_fake_move],
    prompt=ChatPromptTemplate.from_messages([
        ("system", """你现在是一个篮球战术执行分析员,你现在分析的是三人篮球战术
我这里有个记录三人战术视频球员位置的json文件(frame代表视频每帧,S1,S2,S3分别代表标准视频中的三个球员的坐标,T1,T2,T3分别代表学生测试视频中的三个球员的坐标),里面记录了标准视频和学生测试视频中球员在每个帧的位置。我给你一份标准视频球员分析指标,你按照这个指标并根据球员的坐标分析学生测试视频中球员(T1,T2,T3)的战术执行情况。(因为模型检测ID分配不一样,S1对应T1,S2对应T3,S3对应T2球员,分析时需要将S1,S2,S3对应到T1,T3,T2)
分析补充(十分重要,分析时一定要考虑):
1.标准视频和学生测试视频中球员所站的位置并不相同,如果需要跟原位置比较以各自坐标的第一帧进行比较(球员T2在39帧才到指定战术位置,所以他39帧之前的位置变化直接忽略!!!)若连续多帧的帧间速度(两帧间欧式距离)小于 3 像素,即视为“静止”。
2.帧间速度大于5视为跑动
...
"""),
        ("system", """
        你目前可以使用四个工具：read_csv,calc_speed,calc_distance_with_target,read_csv用于读取生成的csv文件,calc_speed用于读取json文件后计算球员速度并返回csv文件,calc_distance_with_target用于读取json文件后计算球员与篮筐的距离并返回csv文件,calc_distance_with_player用于计算球员与目标球员的距离并返回csv文件,detect_fake_move用于读取json文件检测球员是否完成假动作,并返回csv文件.json
        """),
        ("user", "{messages}")
    ])
)
...
def print_stream(stream, save_path: str = "analysis_result.txt"):
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        for s in stream:
            ...
            # 添加分隔符以区分不同的消息
            f.write('-' * 80 + '\n')
print_stream(agent.stream(input={"messages": [HumanMessage(content="json文件路径是frames_output_new.json，你需要三个球员每一帧的数据,根据需要的数据你可以调用不同的工具，这些工具会生成不同的csv文件，你可以根据生成的csv文件名来读取文件并进行分析,请告诉我T1球员在多少帧的时候开始跑动并告诉我速度是多少")]
