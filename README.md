...
## 6.27 version:0.3
<img width="817" alt="截屏2025-06-27 17 14 27" src="https://github.com/user-attachments/assets/dbe71acc-2f74-48a2-89dd-a9847dc5d7e9" />

### 主要更新
1.增加了一个新的工具用于计算球员每次跑动的初始帧和结束帧，对于球员跑动的判定变得更加精细
2.更新提示词，对于每个球员的分析指标有了更加明确的定义。每个球员都有各自特定的分析步骤
### 主要成果
<img width="508" alt="截屏2025-06-27 16 51 08" src="https://github.com/user-attachments/assets/9bdff3b9-ff84-468f-9ee4-a2dac4defce6" />

由此可见模型分析能力已经逼近网页gpt的水平
当前模型分析能力显著提高，相比于0.1版本来说token的消耗也大幅减少

### 主要问题（未解决）：
<img width="471" alt="截屏2025-06-27 11 15 44" src="https://github.com/user-attachments/assets/bb1f43aa-20e8-463f-9cb7-98370351e3fb" />

  模型在读取文件后给出错误信息，模型在单独读取这个文件时可以返回正确信息。但是在分析多个文件后模型就会分析出现错误
  在模型返回中明确说了是根据`basket_distance_result.csv分析得出，那既然从正确文件分析出来为什么还是会分析错误？
  尝试解决方法：在提示词中着重强调（你必须严格基于文件中的实际数据给出结论。如果数据不清晰或有疑问，必须明确指出。禁止编造或推测不存在的数据。在给出结论时必须引用具体数据，提供数据的来源和具体数值。）不稳定，有时能成功。
  尝试解决方法：创建一个工具用于每次生成数据报告时与文件内容进行比对，如果不符合则重新须重新使用工具直至符合标准
